...
pub struct ReplayDelegateMaker {
    pub server_link: Weak<Mutex<ServerState>>,
}
...
macro_rules! make_a_delegate {
    ($self:ident, $id:ident, $loc:ident, $del:ty, $converted:expr) => {{
        ...
        let mut app_lock = app_ptr.lock().unwrap();

        let reference = app_lock.$loc.new_component($converted);

        Box::new(<$del>::new($id, reference))
    }};
}

impl DelegateMaker for ReplayDelegateMaker {
    fn make_method(
        &mut self,
        id: MethodID,
...
impl DocumentDelegate for ReplayDocDelegate {
    ...
    fn on_document_update(
        ...
        let mut app_lock = app_link.lock().unwrap();

        app_lock.update_document(ServerDocumentUpdate {
